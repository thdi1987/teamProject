						...
						$("#info")
								.on(
										...
										function() {
											...
											window
													.open("${pageContext.request.contextPath}/info/");
	...
	<div id="wrapper">
		...
		<ul
			...
			<li class="nav-item"><c:if
					test="${!empty authInfo and authInfo.division == 'S1'}">
					<a href="<c:url value="/Notice/listSys"/>">공지사항</a>
				</c:if></li>

			<li class="nav-item"><c:if
					test="${!empty authInfo and authInfo.division == 'S1'}">
					<a href="<c:url value="/ParcelOut/listSys"/>">분양신청내역</a>
				</c:if></li>

			<li class="nav-item"><c:if
					test="${!empty authInfo and authInfo.division == 'S1'}">
					<a href="<c:url value="/adopt/listSys"/>">입양신청내역</a>
				</c:if></li>

			<li class="nav-item"><c:if
					test="${!empty authInfo and authInfo.division == 'S1'}">
					<a href="<c:url value="/Buyer/listSys"/>">분양자등록신청내역</a>
				</c:if></li>

			<li class="nav-item"><c:if
					test="${!empty authInfo and authInfo.division == 'S1'}">
					<a href="<c:url value="/Quest/listSys"/>">문의사항</a>
				</c:if></li>
				...
				<div class="container-fluid">
					<section id="container">
						<div
							...
							<h1 class="h3 mb-0 text-gray-800">분양 목록</h1>
						</div>
							...
							<table class="table table-hover">
								...
								<c:forEach var="board" items="${boardList2}" varStatus="loop">
									...
									<tr>
										<td>${board.MPNO}</td>
										<td><a href=#
											onclick="window.open('${pageContext.request.contextPath}/ParcelOut/info/${board.MPNO}','_blank','width=1000px,height=800px,toolbars=no' )">
												<img src="<c:url value="/upload/${board.PFILENAME}"/>"
												width="130px" height="130px" />
										</a></td>
										<%-- <c:url value="/upload/ --%>
										<td><a href="#"
											onclick="window.open('${pageContext.request.contextPath}/ParcelOut/info/${board.MPNO}','_blank','width=1000px,height=800px,toolbars=no' )">${board.MPTITLE}</a></td>
							...
							<ul class="pagination">
								<c:if test="${pageMaker.prev}">
									...
									<c:if
										...
										<li><a
											href="${pageContext.request.contextPath}/ParcelOut/listMyPage${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
									</c:if>
								</c:if>
								...
								<c:forEach begin="${pageMaker.startPage}"
									...
									<c:if
										...
										<li
											<c:out value="${pageMaker.cri.page == idx ? 'class=info' : ''}" />>
											<a
											href="${pageContext.request.contextPath}/ParcelOut/listMyPage${pageMaker.makeSearch(idx)}">${idx}</a>
										</li>
									</c:if>
								</c:forEach>
								...
								<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
									...
									<c:if
										test="${empty authInfo or !empty authInfo and authInfo.division == 'A1' or !empty authInfo and authInfo.division == 'B1'}">
										<li><a
											href="${pageContext.request.contextPath}/ParcelOut/listMyPage${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
									</c:if>
								</c:if>
							</ul>
							</div>
